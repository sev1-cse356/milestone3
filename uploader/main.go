...
func process(rdb *redis.Client, data UploadRequest) {
	...
	splitCmd := exec.Command("ffmpeg",
		...
		"-init_seg_name", fmt.Sprintf(`%s_chunk_$RepresentationID$_init.m4s`, data.Id), // Initial segment name
		"-media_seg_name", fmt.Sprintf(`%s_chunk_$RepresentationID$_$Bandwidth$_$Number$.m4s`, data.Id), // Media segment name
